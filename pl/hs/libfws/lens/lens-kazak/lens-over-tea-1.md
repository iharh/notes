...
[School of Haskell tutorial on lenses][SoH basic lensing].

[haskellforall lenses]: http://www.haskellforall.com/2013/05/program-imperatively-using-haskell.html
[SoH basic lensing]: https://www.fpcomplete.com/school/to-infinity-and-beyond/pick-of-the-week/basic-lensing
...
**Disclaimer #3:** this is an exploration more than an explanation, so don't
expect the text to be structured the way normal blog posts are. If I knew how
to structure it all in such a way that it wouldn't be a tangled braindump, I
would've done that.
...
in the Big Value. (Admittedly, it's a *sli-ightly* simplistic view, but it'll
...
While we're at it, let's write our first lens – the `_1` lens— kidding, it's
a kinda boring lens and definitely overused in lens tutorials. Instead, we'll
write the `ix` lens, which accesses the i-th element of a list.
...
~~~
  ...
  * [`second`] is a function from `Data.Bifunctor` (see the
    [bifunctors](@hackage) package), which can be used to apply a function to
    the 2nd element of a tuple (among other things). The line
  ...
  * We could use `second` from `Control.Arrow` instead (because it's in
    base), but I chose `Data.Bifunctor` because bifunctors are much easier to
    understand than arrows. 
...
> Hm... what about making `modifier` return the value before the
> modification? Then `getter` isn't even needed.

To get the value using `ix`, it's enough to give our lens `id` as the
...
Can we do be— sorry, yeah, not funny anymore (never was, in fact). I'll just
tell you that yes, we can do better by changing the `Lens` type again:
...
(if you don't – read in [LYAH][LYAH list monad]), and this is how the type of
`ix` looks now when it's been specialised to use the list monad:
...
wait. These all are functors. Do we really need `Monad` here?

## To hell with monads, functors to the rescue!

Ye-eah, we don't. `Functor f` is quite enough to implement `ix` and do fun
stuff with it – and why should we ask for more? (Spoiler: we will ask for
more... but not until we need it.)
...
Meet `Storey`. This is a *functor modifier* – it does what the original
functor does, but it also attaches a value to it.
...
Now I'll show how to use it to get the original value. The trick is for the
update function to attach the value before modification to the functor – and
then it will get, unmodified, into the “outer world” (from `f a` to `f s`).
...
And now look how `Storey` lets us easily get the original value as well as
the updated value:

~~~ haskell
> ix 2 (\x -> Storey x [1..x]) [300, 100, 4, 600, 900, 400]
SF 4 [ [300, 100, 1, 600, 900, 400]
     , [300, 100, 2, 600, 900, 400]
     , [300, 100, 3, 600, 900, 400]
     , [300, 100, 4, 600, 900, 400] ]
~~~
...
use [`Const`], which is like our `Storey` but without the functor part – and
it's also in the base libraries. Compare: this is the `Storey` version of a
function which uses a lens to extract some value:
...
Bonus stuff:
  ...
  * [`second`] from `Data.Bifunctor` or `Control.Arrow` maps over the second
    element of a tuple.
...
to illustrate what I'm talking about:
  ...
  * We can make a lens for focusing on several elements in the list, as long
    ...
    ~~~ haskell
    > let launchMissiles = putStrLn "kaboom!"
    ~~~
...
works, skip this section; otherwise, read on.
  ...
  * `Functor` allows acting on a value inside the functor:
...
~~~ haskell
-- update :: a -> f a
update old = if old == ref then f old then pure old
~~~
...
_all' ref f s = mapM (\old -> if old == ref then f old else return old) s
~~~
...
preview
  ...
  -> s
  -> Maybe a
preview l = getFirst . getConst . l (\x -> Const (Just x))
~~~
...
do a lot of them, and it's called *difference lists*. I don't like the
explanations there are (e.g. in [Real World Haskell][RWH dlist] and on
[StackOverflow][SO dlist]), so I'll explain it myself.
...
(in a nutsell: use [`Data.Tree`], generate the “evil tree” recursively,
...
The important thing is that with difference lists we do not more preliminary
steps as there are lists to append; after that each element is generated in
constant time. With ordinary lists, each element takes the time proportional
to its depth in the tree – and this leads to quadratic behavior.
...
Um, I'm sorry for using 2 exclamation marks in a paragraph. I just thought it
...
Ouch, sorry. Our `AppLens` is actually called [`Traversal`] in lens library!
Here's the All-New Rebranded Definition:
...
(The `s -> a, ...` bit is needed so that the compiler would be able to infer
types for us (go on, try to delete it – you'll get a bunch of “ambiguous
type” errors when you'll try to actually use `each`).)
...
doesn't have [type-level lambdas][], any why is *this* I, frankly, have
absolutely no idea, but I have a vague feeling that explaining this
adequately would require another series of posts... so maybe later.)
...
[`DefaultSignatures`](@ghcext) extension, which HLint apparently isn't yet
...
explanation [hm, or should I call it “bullet list explanation”?]:
  ...
  * You can give -methods of typeclasses- default implementations. It allows
    instance writers use the -implementation they would've used anyway-
...
## Recap
  ...
  * There are various predefined traversals, such as `each`, `_head` and
    `_last`. (Another few easy ones are [`both`], [`_tail`] and [`_init`].
    Other ones use scary indexed stuff and [`Bazaar`]s and
...
I'll start writing right after I manage to find 50$ to pay rent for this
week. This means that if e.g. I get 50$ in donations by tomorrow, I'll start
writing tomorrow. (If I don't, well, maybe the next week. Or the week after
that, I don't know. Most likely I'll have to find a job in McDonald's or
something, and this wouldn't leave me much free time or energy to write
anything.)

[currently donated: <u>0$</u> (unsurprisingly)]

<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
<input type="hidden" name="cmd" value="_s-xclick">
<input type="hidden" name="hosted_button_id" value="PG2PLULRYTGHE">
...
[`Identity`]: http://hackage.haskell.org/package/transformers/docs/Data-Functor-Identity.html
